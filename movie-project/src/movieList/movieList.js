...
function MovieList(props) {
  const domConfettiRefs = props.movieData.map(() => React.createRef());
      ...
      return (
        <StyledDiv key={idx} backgroundImage={unit.image} likeImage={'/images/like.svg'}>
          ...
          <div className="like-button">
            <span ref={domConfettiRefs[idx]} onClick={() => showParadise(idx)}></span>
          </div>
        </StyledDiv>
      )
    })
  ...
  return (
    <div>{renderMovieList(props.movieData)}</div>
  );
}
